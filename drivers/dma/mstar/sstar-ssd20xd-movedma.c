...
struct ssd20xd_movedma {
	...
	/* watchdog to workaround stuck dma */
...
static void ssd20xd_movedma_markinflightfinished(struct ssd20xd_movedma *movedma)
{
	if (movedma->inflight) {
...
static void ssd20xd_movedma_do_single(struct ssd20xd_movedma_chan *chan, struct ssd20xd_movedma_desc *desc)
{
	...
	mod_timer(&movedma->watchdog, jiffies + msecs_to_jiffies(500));
...
static int ssd20xd_movedma_probe(struct platform_device *pdev)
{
	...
	timer_setup(&movedma->watchdog, ssd20xd_movedma_watchdog, 0);

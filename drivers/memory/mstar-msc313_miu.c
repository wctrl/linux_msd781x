		...
		struct msc313_miu *miu)
{
	...
	clk_init = devm_kzalloc(dev, sizeof(*clk_init), GFP_KERNEL);
	if (!clk_init)
		return -ENOMEM;
	...
	miu->ddrpllparents[0] = of_clk_get_parent_name(dev->of_node, 1);
	clk_init->parent_names = miu->ddrpllparents;

	clk = clk_register(dev, &miu->clk_hw);
	if (IS_ERR(clk))
		return -ENOMEM;
...
static int msc313_miu_probe(struct platform_device *pdev)
{
	...
	miu->analog = devm_regmap_init_mmio(dev, base,
	...
	miu->digital = devm_regmap_init_mmio(dev, base,
			&msc313_miu_digital_regmap_config);
	if (IS_ERR(miu->digital))
		return PTR_ERR(miu->digital);

	miu->miuclk = devm_clk_get(dev, "miu");
	if (IS_ERR(miu->miuclk))
		return PTR_ERR(miu->miuclk);

	miu->ddrreg = devm_regulator_get_optional(dev, "ddr");
	if (IS_ERR(miu->ddrreg)) {
		int err = PTR_ERR(miu->ddrreg);
		/*
		 * If the DDR regulator is connected to something that needs
		 * miu (like i2c) then we get a dependency loop here.
		 * If the regulator isn't ready yet assume the driver isn't
		 * ready but the regulator is putting out the juice.
		 */
		if (err == -EPROBE_DEFER)
			dev_info(dev, "DDR regulator is not ready yet..\n");
		else {
	...
	dev_info(dev, "Memory type is %s, %d banks and %d columns, %d bit bus", types[config1 & REG_CONFIG1_TYPE],
			banks, cols, buswidth);
	...
	dev_info(dev, "trcd: %d, trp: %d, tras: %d, trrd: %d, trtp: %d, trc: %d",

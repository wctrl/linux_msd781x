...
static int msc313_isp_setup(struct spi_device *spi)
{
	...
	printk("hohoho %d \n", (int) clk_round_rate(isp->pm_spi_clk, spi->max_speed_hz));
...
{
	int i;
	struct msc313_qspi_readmode *readmode = NULL;
	...
	for (i = 0; i < ARRAY_SIZE(opcode_mapping); i++){
		if (opcode_mapping[i].opcode == desc->info.op_tmpl.cmd.opcode){
			readmode = &opcode_mapping[i];
			break;
		}
	}

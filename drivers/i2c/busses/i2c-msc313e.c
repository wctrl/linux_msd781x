...
static struct reg_field ctrl_rst_field = REG_FIELD(REG_CTRL, 0, 0);
static struct reg_field ctrl_enint_field = REG_FIELD(REG_CTRL, 2, 2);
static struct reg_field startstop_start_field = REG_FIELD(REG_STARTSTOP, 0, 0);
static struct reg_field startstop_stop_field = REG_FIELD(REG_STARTSTOP, 8, 8);

static struct reg_field wdata_data_field = REG_FIELD(REG_WDATA, 0, 7);
static struct reg_field wdata_ack_field = REG_FIELD(REG_WDATA, 8, 8);

static struct reg_field rdata_data_field = REG_FIELD(REG_RDATA, 0, 7);
static struct reg_field rdata_trigack_field = REG_FIELD(REG_RDATA, 8, 9);

static struct reg_field status_state_field = REG_FIELD(REG_INT_STAT, 0, 4);
static struct reg_field status_int_field = REG_FIELD(REG_INT_STAT, 8, 13);
...
static int msc313e_i2c_probe(struct platform_device *pdev)
{
	...
	msc313ei2c->clk = of_clk_get(pdev->dev.of_node, 0);
	clk_prepare_enable(msc313ei2c->clk);

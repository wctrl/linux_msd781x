...
static int ea3056_calculate_voltage(struct ea3056_regulator_config *regconfig, unsigned int vref)
{
	...
	/* work out the ratio of the select resistors, result multiplied by 1000 */
	tmp = (regconfig->vselr1 * 1000) / regconfig->vselr2;
	/* multiply vfb multiplied by 1000 by the ratio */
	tmp = (vref * 1000) * tmp;
	/* rescale back to mV */
	tmp /= 1000000;
	/* Add the fixed part */
...
static int ea3056_get_voltage(struct regulator_dev *rdev)
{
	...
	/* The vref register only matter if it's enabled */
	if (reg & VREF_CTRL)

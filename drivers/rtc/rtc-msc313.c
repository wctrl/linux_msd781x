...
{
	struct msc313_rtc *info = dev_get_drvdata(dev);
...
{
	struct msc313_rtc *info = dev_get_drvdata(dev_id);
	u16 reg;
...
{
	struct msc313_rtc *info;
	struct clk *clk;
	int ret = 0, irq;
	u16 reg;
	u32 rate;

	info = devm_kzalloc(&pdev->dev, sizeof(struct msc313_rtc), GFP_KERNEL);
	if (!info)
		return -ENOMEM;

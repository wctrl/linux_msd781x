...
static struct clk_hw *msc313_mux_xlate(struct of_phandle_args *clkspec, void *data)
{
...
int msc313_mux_register_muxes(struct device *dev, struct regmap *regmap, const struct msc313_muxes_data *muxes_data)
{
	...
	for (i = 0; i < muxes_data->num_muxes; i++, mux++, mux_data++) {
		...
		mux_init.parent_data = mux_data->parents;
